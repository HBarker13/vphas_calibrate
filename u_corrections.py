...
#use only the best stars ie. good photometry, magnitudes ~13 to ~17
...
def hist_2d(x,y, savechoice):
    ...
    #nxbins = int(max(x)/0.03)
    #nybins = int(max(y)/0.08)
    ...
    plt.xlabel('g-r')
    plt.ylabel('u-g')
   #plt.title('block '+block_choice+' '+ap_name)
    ...
    #if u_txtline!=' ':
   # 	plt.annotate(u_txtline, xy=(1.5, -0.9))
    	
    if savechoice==True:	
    	savepath = os.getcwd()+'/'+block_choice+'_u_'+ap_name+'_correction.pdf'
    	plt.savefig(savepath)
    	print 'Saved'
    elif savechoice==False:
   	 plt.show()
    plt.close
...
def calc_colours(table_fpath):
	...
	u_mags = [line if 12<line<19 else float('nan') for line in u_mags]
	g_mags = [line if 12<line<19 else float('nan') for line in g_mags]
	#r_r_mags = [line if 12<line<19 else float('nan') for line in r_r_mags]
	r_b_mags = [line if 12<line<19 else float('nan') for line in r_b_mags]
...
print 'Number of points: ', len(u_min_g)


u_txtline=' '
hist_2d(g_min_r, u_min_g, False)


#loop over values to shift the u band
running_val = 0
loop=True
while(loop):
	...
	if running_val<0:
		...
		u_txtline = 'u = u + ' + str(running_val)
	
	hist_2d(g_min_r, u_min_g, False)
	
	exit = raw_input('Press q to exit and add corrected u magntiudes to the bandmerged fits file or any other key to continue adjusting magnitudes: ')
	if exit=='q' or exit=='Q':
		loop=False
...
if not ap_name+'_corr_'+u_appendix in table.dtype.names:
	u_mags = table[ap_name+'_mag_'+u_appendix]
	new_u = [line+running_val for line in u_mags]
...
if not ap_name+'_corr_AB_'+u_appendix in table.dtype.names:
	...
	new_AB = [line+running_val for line in u_AB]
...
txt_fpath = os.getcwd()+'/u_corrections_'+block_choice+'.txt'
with open(txt_fpath, 'a') as txtfile:
	#aperture u_corrections error, negative to go with rest of my aperture corrections
