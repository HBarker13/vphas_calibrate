...
#for ap_rad in range(3,4):
for ap_rad in range(2,6):
	ap_name = 'Aper_flux_'+str(ap_rad)
	print ap_name
	

	#synthetic tracks are in vega, so start with vega u mags
	u_mags = table[ap_name+'_mag_'+u_appendix]

	
	#aperture corrected mags for the other filters
	...
	while gaussian_loop == True:
		...
		if chi > prev_chi and (0.95*max_sum)<popt[0]<(1.05*max_sum):
			...
			#plot the optimal gaussian fit found
			#plt.figure()
			#plt.plot(x, y, 'bo')
			#plt.plot(x, gaus_fit, 'r')
			#plt.show()
	...
	#popt = [a, mean, sigma]
	optimal_u_shift = prev_popt[1] #=mean of the optimal gaussian fit
	...
	#left side of the Gaussian peak = u shifts more negative than the optimal value
